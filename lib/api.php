...
class FastlyAPI {
  ...
  /**
   * Sends a surrogate key purge request to the Fastly API.
   * @param $key Surrogate_key to purge.
   */
  function purge_surrogate_key($key) {
    // TODO How can we handle this more elegantly?
    if (!$this->api_key)
      return;

    if (!get_option('fastly_service_id') )
      return;

    // TODO - change this to a curl_multi_exec at some point
    $logPurges = (bool)get_option('fastly_log_purges');
    if( $logPurges ) {
        error_log("Purging surrogate key " . $key);
    }
    ...
    $url .= '/service/' . get_option('fastly_service_id') . '/purge/' . $key;
